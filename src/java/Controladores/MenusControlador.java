...
public class MenusControlador {
    ...
    public static boolean eliminar(Menus menu) {
     ...
     public static Menus buscarNombreRepetidoMenu(Menus menu) {
        if (Conexion.conectar()) {
            String sql = "select * from menus where nombre_menu='"
                    + menu.getNombre_menu() + "'";
            System.out.println(sql);
            try {
                ...
                if (rs.next()) {
                    ...
                    menu.setNombre_menu(menu.getNombre_menu());
                }
            } catch (SQLException ex) {
